...
object BlockListener : SLEventListener() {
	...
	@EventHandler
	fun portalPhysics(event: BlockPhysicsEvent) {
		if (event.block.type == Material.END_PORTAL) event.isCancelled = true
	}

	// Don't allow breaking blocks with custom items
	@EventHandler
	fun onBreakWithCustomItem(event: BlockBreakEvent) {
	...
	@EventHandler
	fun onBrownMushroomGrow(event: StructureGrowEvent) {
	...
	fun onMobDefenderBreak(event: BlockBreakEvent) {
