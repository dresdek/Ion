...
object GameplayTweaks : SLComponent() {
	override fun onEnable() {
		...
		plugin.listen<BlockPhysicsEvent> { event -> if (physicsDisabled) event.isCancelled = true }
		plugin.listen<ItemSpawnEvent> { event -> event.entity.isInvulnerable = true }
	}
	...
	private fun modifyBlastResistance() {
		...
		plugin.listen<EntityExplodeEvent>(priority = EventPriority.LOWEST) { event ->
			event.blockList().removeIf { it.isLiquid }
		}
		plugin.listen<BlockExplodeEvent>(priority = EventPriority.LOWEST) { event ->
			event.blockList().removeIf { it.isLiquid }
		}
	}
