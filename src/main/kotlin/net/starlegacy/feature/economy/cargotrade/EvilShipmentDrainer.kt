...
object EvilShipmentDrainer {
	class ShipmentBundle(val shipment: CargoCrateShipment) {
		...
		val averageRevenue get() = shipments.sumByDouble { it.crateRevenue } / shipments.size
		val lowestRevenue get() = shipments.minByOrNull { it.crateRevenue }?.crateRevenue ?: 0.0
		val totalSoldCrates get() = shipments.sumBy { min(it.totalCrates, it.soldCrates) }
	...
	private fun drainUser(playerId: SLPlayerId, list: List<ShipmentBundle>, forReal: Boolean) {
		...
		if (extraMoney > 0) {
			...
			val totalMoney = list.sumByDouble { it.averageRevenue * it.totalSoldCrates }
	...
	private fun getExtraMoney(list: List<ShipmentBundle>): Double = list.sumByDouble(this::getExtraMoney)
