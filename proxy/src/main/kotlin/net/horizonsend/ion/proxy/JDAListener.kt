	...
	override fun onMessageReceived(event: MessageReceivedEvent) {
		if (event.isFromType(ChannelType.PRIVATE)) {
			if (event.message.contentRaw.length == 4) {
				...
				} else event.message.reply("Link code is invalid, non-existent, or expired.").queue()
			} else event.message.reply("Message is too short or too long to be a verification code. Please resend the code.").queue()
		}
	}
}